...
flow_diagram_enhanced <- PRISMA_flowdiagram_modified(
  data = prisma_data,
  interactive = FALSE,
  previous = FALSE,
  other = TRUE,
  ...
  fontsize = 12,
  font = "Aptos",
  # Customized fields added to PRISMA flowchart.
  citations_duplicates = 209,        # Duplicates removed from citations
  citations_screened = 588,          # Citations after deduplication  
  citations_excluded_asreview = 546  # Citations excluded by ASReview
)

# Save as html 
PRISMA_save(
  flow_diagram_enhanced, 
  filename = paste0(figfolder,"/PRISMA_PoC.html"), 
  filetype = "html",
  overwrite = TRUE
)
...
pagedown::chrome_print(
  paste0(figfolder,"/PRISMA_PoC.html"), 
  output = paste0(figfolder,"/PRISMA_PoC.pdf"),
  options = list(
    landscape = TRUE,
    format = "A4"  # optional: specify paper format
  )
)

# Save as png
webshot2::webshot(
  url = paste0(figfolder,"/PRISMA_PoC.html"), 
  file = paste0(figfolder,"/PRISMA_PoC.png"),
  vwidth = 1100,
  vheight = 830,
  zoom = 5
)